...
cmsDriver.py Configuration/GenProduction/python/${HADRONIZER} \
    ...
    --era Run2_2017 --nThreads $nthreads \
...
# Doing HLT step in CMSSW CMSSW_9_4_14_UL_patch1
cd ${BASEDIR}/CMSSW_9_4_14_UL_patch1/src
eval `scram runtime -sh`
scram b -j 4
echo "########################################################################################"
echo "########################################################################################"
...
cmsDriver.py step1 \
    ...
    --step HLT:2e34v40 --geometry DB:Extended \
...
remoteDIR="/store/group/lpcmetx/iDMe//Samples/signal_privateMC/2017/"
