    ...
    for p in points:
        if skimmed:
            ...
            if 'mZD' in p:
                ...
                mzd = ""
            status, lifetimes = xrdClient.dirlist(f"{prefix}/{year}/sig_{p}")
            lifetimes = [l.name.split("_")[-1] for l in lifetimes]
            
            for l in lifetimes:
                try:
                    ct = int(l.split("-")[1])
                    info = {}
                    #info["location"] = f"{prefix}/{year}/sig_{p}/{p}_{l}/"
                    info["Mchi"] = mchi
                    info["dMchi"] = dmchi
                    info["ctau"] = ct
                    if mzd != "":
                    ...
                    target_dir = f"{prefix}/{year}/sig_{p}/{p}_{l}/"
