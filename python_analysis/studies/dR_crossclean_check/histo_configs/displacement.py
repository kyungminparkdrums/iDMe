...
def make_histograms():
    histograms = {
        ...
        "sel_vtx_vx": Hist(samp,cut,vx,storage=hist.storage.Weight()),
        "sel_vtx_vy": Hist(samp,cut,vy,storage=hist.storage.Weight()),
        ...
        "gen_cos_collinear": Hist(samp,cut,cos_collinear,storage=hist.storage.Weight()),
        ...
        "sel_vtx_vx_vs_sel_vtx_vy" : Hist(samp,cut,vx,vy,storage=hist.storage.Weight()),
...
def fillHistos(events,histos,samp,cut,info,sum_wgt=1):
    ...
    histos["sel_vtx_vx"].fill(samp=samp,cut=cut,vx=vtx.vx,weight=wgt)
    histos["sel_vtx_vy"].fill(samp=samp,cut=cut,vy=vtx.vy,weight=wgt)

    histos["sel_vtx_pt_e1_over_pt_e2"].fill(samp=samp,cut=cut,ratio=e1.pt/e2.pt,weight=wgt)
    ...
    histos['sel_vtx_vx_vs_sel_vtx_vy'].fill(samp=samp,cut=cut,vx=vtx.vx,vy=vtx.vy,weight=wgt)
    ...
    '''
    if info["type"] == "signal":
        #histos['gen_cos_collinear'].fill(samp=samp,cut=cut,cos_collinear=events.gen_cos_collinear,weight=wgt)
        #histos['gen_chi2_pt'].fill(samp=samp,cut=cut,chi2_pt=events.gen_chi2_pt,weight=wgt)
        #histos['gen_ee_pt'].fill(samp=samp,cut=cut,pt=events.gen_ee_pt,weight=wgt)
        #histos['gen_chi2_ee_pt_ratio'].fill(samp=samp,cut=cut,ratio=events.gen_ee_pt/events.gen_chi2_pt,weight=wgt)

        #histos['gen_chi2_pt_vs_gen_ee_pt'].fill(samp=samp, cut=cut, chi2_pt = events.gen_chi2_pt, pt = events.gen_ee_pt, weight=wgt)
