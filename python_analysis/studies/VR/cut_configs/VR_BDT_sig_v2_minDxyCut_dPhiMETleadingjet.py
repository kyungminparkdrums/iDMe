...
def cut4(events,info):
    ...
    if len(events) != 0:
        input = routines.makeBDTv5Inputs(events)
    
        model = './models/BDT_sig_v2_minDxyCut_10Vars_comb11_max_depth_6_n_estimators_500_lr_0.01.json'
...
def cut8(events,info):
    name = "cut8"
    desc = "0.75 < dPhi(MET, leading jet) < 1.5"
    plots = True
    cut = (np.abs(events.PFJet.METdPhi[:,0]) > 0.75) & (np.abs(events.PFJet.METdPhi[:,0]) < 1.5)
    return events[cut], name, desc, plots
