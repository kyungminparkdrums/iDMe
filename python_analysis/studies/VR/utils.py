...
def plot_signal_efficiency(sig_histo, df, m1s, deltas, ctaus, doLog = True, ylabel = '', title = ''):
    cuts = get_signal_list_of_cuts(sig_histo)
...
def plot_bkg_efficiency(bkg_histos, df, doLog = True, ylabel = '', title = ''):
    ...
    cuts = get_bkg_list_of_cuts(bkg_histos)
...
def plot_bkg_1d(ax, bkg_histos, plot_dict, style_dict, processes = 'all'):  
    ...
    if processes == 'all':
        ...
        list_cut_index = get_bkg_list_of_cuts(bkg_histos, get_cut_idx=True)
        list_cut_name = get_bkg_list_of_cuts(bkg_histos, get_cut_idx=False)
        
        cut_name = plot_dict['cut']
        
        df = get_bkg_cutflow_df(bkg_histos, 'cutflow_cts').iloc[:-1]
...
def plot_bkg_1d_stacked(ax, bkg_histos, plot_dict, style_dict, processes = 'all'):  
    ...
    # Plot
    mplhep.histplot(bkg_stack, yerr=style_dict['doYerr'], density=style_dict['doDensity'], ax=ax, histtype='step', flow=style_dict['flow'], label = style_dict['label'])
...
def plot_bkg_2D(ax, bkg_histos, plot_dict, style_dict, processes = 'all'):  
    ...
    # Plot
    if style_dict['doLogz']:
        mplhep.hist2dplot(bkg_stack, flow=style_dict['flow'], norm=mpl.colors.LogNorm(), ax=ax)
    else:
        mplhep.hist2dplot(bkg_stack, flow=style_dict['flow'], ax=ax)
