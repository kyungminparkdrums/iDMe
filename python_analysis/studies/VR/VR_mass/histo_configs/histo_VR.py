...
def make_histograms():
    histograms = {
        ...
        "sel_vtx_mass_refit": Hist(samp,cut,mass,storage=hist.storage.Weight()),
        "sel_vtx_mass_refit_delta": Hist(samp,cut,ratio_big,storage=hist.storage.Weight()),
        "sel_vtx_mass_vs_sel_vtx_mass_refit" : Hist(samp,cut,mass,mass_refit,storage=hist.storage.Weight()),
...
def fillHistos(events,histos,samp,cut,info,sum_wgt=1):
    ...
    histos["sel_vtx_mass_refit_delta"].fill(samp=samp,cut=cut,ratio_big=np.abs((vtx.refit_m-vtx.m)/vtx.m),weight=wgt)
    histos["sel_vtx_mass_vs_sel_vtx_mass_refit"].fill(samp=samp,cut=cut,mass=vtx.m,mass_refit=vtx.refit_m,weight=wgt)
