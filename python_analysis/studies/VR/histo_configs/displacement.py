...
def make_histograms():
    histograms = {
        ...
        "jet_met_dPhi" : Hist(samp,cut,dphi,storage=hist.storage.Weight()),
...
def fillHistos(events,histos,samp,cut,info,sum_wgt=1):
    ...
    #histos["jet_met_dPhi"].fill(samp=samp,cut=cut,dphi=ak.flatten(np.abs(events.PFJet.METdPhi[:,:])),weight=wgt)
