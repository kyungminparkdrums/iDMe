...
def cut5(events,info):
    name = "cut5"
    desc = "Veto jets (pT > 100, eta [2.25,3.0])"
    plots = True

    hasVetoJet = (events.PFJet.pt[:,:] > 100) & ((np.abs(events.PFJet.eta[:,:]) < 3.0) & (np.abs(events.PFJet.eta[:,:]) > 2.25))
    #print(ak.sum(hasVetoJet, axis=1))
    hasVetoJet = ak.any(hasVetoJet, axis=1)
    
    #print(events.PFJet.pt[:,0] > 80)

    cut = ~hasVetoJet
    
    print(f'Veto jets Pass: {np.count_nonzero(cut)}/{len(cut)}')
