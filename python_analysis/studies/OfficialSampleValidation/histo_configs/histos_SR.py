...
def make_histograms(info):
    ...
    h.make("sel_vtx_vxy10_fromPV_vectorSub",'vxy10')
    ...
    h.make("bdtscore_vs_MET",'bdtscore', 'met1000')
    h.make("bdtscore_vs_ee_pT",'bdtscore', 'vtx_pt_refit')
...
def fillHistos(events,h,samp,cut,info,sum_wgt=1):
    ...
    #h.fill("sel_e1_dxy_fromPV", dxy=sel_vtx.e1.dxy-np.sqrt(events.PV.x*events.PV.x + events.PV.y*events.PV.y),weight=wgt)
    #h.fill("sel_e2_dxy_fromPV", dxy=sel_vtx.e2.dxy-np.sqrt(events.PV.x*events.PV.x + events.PV.y*events.PV.y),weight=wgt)
    ...
    h.fill('sel_vtx_ThetaColl_fromPV_refit_rad',theta_rad=np.arccos(sel_vtx.cos_collinear_fromPV_refit),weight=wgt)
    ...
    """
    ## ## 
    h.fill("bdtscore_vs_MET",bdtscore=events.BDTScore, met=events.PFMET.pt,weight=wgt)
    h.fill("bdtscore_vs_ee_pT",bdtscore=events.BDTScore,pt_refit=sel_vtx.refit_pt,weight=wgt)
    """
    ...
    h.fill("sel_vtx_vxy1_fromPV_scalarSub",vxy=sel_vtx.vxy,weight=wgt)
    h.fill("sel_vtx_vxy1_fromPV_vectorSub",vxy=np.sqrt((sel_vtx.vx-events.PV.x)**2+(sel_vtx.vy-events.PV.y)**2),weight=wgt)
    h.fill("sel_vtx_vxy10_fromPV_vectorSub",vxy=np.sqrt((sel_vtx.vx-events.PV.x)**2+(sel_vtx.vy-events.PV.y)**2),weight=wgt)

    #h.fill("sel_e1_dxy_refit", dxy=sel_vtx.e1.refitDxy, weight=wgt)
    
    h.fill("sel_e1_dxy_manual",dxy=dxy_custom(sel_vtx.e1.vxy*np.cos(sel_vtx.e1.phi), sel_vtx.e1.vxy*np.sin(sel_vtx.e1.phi), events.PV.x, events.PV.y, sel_vtx.e1.pt*np.cos(sel_vtx.e1.phi), sel_vtx.e1.pt*np.sin(sel_vtx.e1.phi), sel_vtx.e1.pt),weight=wgt)
    h.fill("sel_e1_dxy_manual_fromPV",dxy=dxy_custom(sel_vtx.e1.vxy*np.cos(sel_vtx.e1.phi)-events.PV.x, sel_vtx.e1.vxy*np.sin(sel_vtx.e1.phi)-events.PV.y, events.PV.x, events.PV.y, sel_vtx.e1.pt*np.cos(sel_vtx.e1.phi), sel_vtx.e1.pt*np.sin(sel_vtx.e1.phi), sel_vtx.e1.pt),weight=wgt)
    
    h.fill("bdtscore",bdtscore=events.BDTScore,weight=wgt)

    h.fill("bdtscore_vs_sel_vtx_CosThetaColl_fromPV_refit",bdtscore=events.BDTScore,\
           cosTheta_fromPV_refit=sel_vtx.cos_collinear_fromPV_refit,weight=wgt)
    h.fill("bdtscore_vs_sel_vtx_CosThetaColl_fromPV",bdtscore=events.BDTScore,\
    ...
    h.fill('bdtscore_vs_sel_vtx_thetaColl_fromPV_refit_rad',bdtscore=events.BDTScore,\
           theta_rad=np.arccos(sel_vtx.cos_collinear_fromPV),weight=wgt)
    '''
    h.fill('bdtscore_vs_sel_vtx_sign',bdtscore=events.BDTScore,\
           sign=sel_vtx.sign,weight=wgt)
    h.fill('sel_vtx_CosThetaColl_fromPV_refit_vs_sel_vtx_sign',cosTheta_fromPV_refit=sel_vtx.cos_collinear_fromPV_refit,\
           sign=sel_vtx.sign,weight=wgt)
    '''
