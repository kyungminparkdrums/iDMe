...
def cut4(events,info):
    ...
    variables = ['sel_vtx_chi2','sel_vtx_METdPhi','sel_vtx_m','sel_vtx_dR','sel_vtx_minDxy','vxy_signif'] # BDT v2 variables

    if len(events) != 0:
        input = routines.makeBDTv2Inputs(events)
    
        model = './models/BDT_ctau-1_mass_5to100.json'
...
def cut10(events,info):
    ...
    # BDT thresholds: 85% signal tagging eff.
    thres = {'1': 0.7561, '10': 0.8773, '100': 0.8253}

    model = {'1': './models/BDT_ctau-1_mass_5to100.json',\
             '10': './models/BDT_ctau-10_mass_5to100.json',\
             '100': './models/BDT_ctau-100_mass_5to100.json'
            }
    ...
    if len(events) != 0:
        ...
        score_BDT = {'1': routines.getBDTscore(input, model['1']),\
                     '10': routines.getBDTscore(input, model['10']),\
                     '100': routines.getBDTscore(input, model['100'])
                    }

        #print('ctau-1 pass: ', score_BDT['1'] > thres['1'])
        #print('ctau-10 pass: ', score_BDT['10'] > thres['10'])
        #print('ctau-100 pass: ', score_BDT['100'] > thres['100'])
        
        cut = (score_BDT['1'] > thres['1']) | (score_BDT['10'] > thres['10']) | (score_BDT['100'] > thres['100'])
        #print(cut)
