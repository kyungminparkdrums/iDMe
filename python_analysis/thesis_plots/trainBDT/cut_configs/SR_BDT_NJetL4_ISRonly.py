...
def cut0(events,info):
    ...
    if len(events) != 0:
        ...
        model = './models/BDT_BJetMedium_NJetL4_ISRonly.json'
...
def cut4(events,info):
    ...
    thres = 0.9957 # for BDT_BJetMedium_NJetL4_ISRonly.json, trained on 0 < Njets < 4 but only w/ ISR cuts on jets as presel
    if len(events) != 0:
...
def cut5(events,info):
    ...
    thres = 0.9980 # for BDT_BJetMedium_NJetL4_ISRonly.json, trained on 0 < Njets < 4 but only w/ ISR cuts on jets as presel
    if len(events) != 0:
        cut = events['BDTScore'] > thres
    else:
        cut = []
...
def cut6(events,info):
    ...
    thres = 0.9996 # for BDT_BJetMedium_NJetL4_ISRonly.json, trained on 0 < Njets < 4 but only w/ ISR cuts on jets as presel
    if len(events) != 0:
        cut = events['BDTScore'] > thres
    else:
        cut = []
